...
class GSynthesis(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        rgb_converters = [
            EqualizedConv1d(nf(1), num_channels, 1, gain=1, use_wscale=use_wscale)
        ]
    ...
    def forward(
        self, dlatents_in, depth=0, alpha=0.0, labels_in=None, const_input=None
    ):
        """
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        latent_size=512,
        dlatent_size=512,
        const_input_dim=512,  # const_input_dim for encoder samples
        conditional=False,
        n_classes=0,
        truncation_psi=0.7,
        fmap_max=512,
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None, const_input=None):
        ...
        if self.training:
            ...
            if self.disease_mixing_prob is not None and self.disease_mixing_prob > 0:
                ...
                mixing_cutoff = (
                    ...
                    if random.random() < self.disease_mixing_prob
                    else cur_layers
                )
        ...
        fake_images = self.g_synthesis(
            dlatents_in, depth, alpha, labels_in=labels_in, const_input=const_input
        )
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        from_rgb.append(
            EqualizedConv1d(
                num_channels, nf(2), kernel_size=1, gain=gain, use_wscale=use_wscale
            )
        )
        self.from_rgb = nn.ModuleList(from_rgb)
        if disease_model is None:
            self.final_classifier = DiscriminatorTop(
                self.mbstd_group_size,
                self.mbstd_num_features,
                in_channels=nf(2),
                intermediate_channels=nf(2),
    ...
    def forward(self, images_in, depth, alpha=1.0, labels_in=None):
...
class Ecgedit:
    ...
    def __init__(
        ...
        self.__setup_gen_optim(learning_rate=self.lr, beta_1=0.0, beta_2=0.99, eps=1e-8)
        self.__setup_dis_optim(learning_rate=self.lr, beta_1=0.0, beta_2=0.99, eps=1e-8)
    ...
    def __setup_gen_optim(self, learning_rate, beta_1, beta_2, eps):
        self.gen_optim = torch.optim.Adam(
            [
                ...
                {"params": self.disease_model.parameters()},
            ],
            lr=learning_rate,
            betas=(beta_1, beta_2),
            eps=eps,
    ...
    def save_ckpt(self, epoch, stage):
        state = {
        ...
        d_state = {
            ...
            "epoch": epoch,
            "optimizer": self.dis_optim.state_dict(),
            "lr": self.lr,
            "stage": stage,
        }
        f_state = {
            "state_dict": self.disease_model.state_dict(),
            "epoch": epoch,
            "lr": self.lr,
            "stage": stage,
        }
    ...
    def optimize_generator(
        ...
        disease, const_disease, q_loss_s, prefix_s, _ = self.disease_model(
            disease_samples, disease_pos, disease_descri
        )
        disease_r, const_r, q_loss_r, prefix_r, _ = self.disease_model(
            real_samples, real_pos, real_descri
        )

        edited_samples = self.gen(disease, depth, alpha, None, const_r)
        recon_samples = self.gen(disease_r, depth, alpha, None, const_r)

        fake_disease, const_f, q_loss_f, prefix_f, _ = self.disease_model(
            edited_samples, real_pos, disease_descri
        )

        loss, f_loss, rec_loss, sim_loss = self.loss.gen_loss(
            real_samples,
            edited_samples,
            recon_samples,
            disease,
    ...
    def optimize_discriminator(
        ...
        return (
            loss_val / self.d_repeats,
            loss_val / self.d_repeats,
            loss_ori / self.d_repeats,
        )
