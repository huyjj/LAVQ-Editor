...
def train(args):
    ...
    train_dataset = PTBXLTestClsdataset('ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/',
                                         True, choose=None, 
                                         train_lis=['work_dir/LAVQ_Editor/ecg',
                                             'ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/Train_sclc_X'
                                             ])
    ...
    val_dataset = PTBXLTestClsdataset('ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/', 
                                      True, choose=['HYP', 'MI', 'CD', 'STTC', 'NORM'],
                                      train_lis=['ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/Patient_Select_145_sclc_X_half2']) 
    val_dataloader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=True,
                                num_workers=4, drop_last=True)
    val_dataset1 = PTBXLTestClsdataset('ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/', 
                                      True, choose=['HYP', 'MI', 'CD', 'STTC', 'NORM'],
    ...
    print_log(f"train_datasize {len(train_dataset)} val_datasize {len(val_dataset)} val_datasize1 {len(val_dataset1)}", model_save_dir)
    ...
    logger = SummaryWriter(logdir=model_save_dir, flush_secs=2)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        logger.add_scalar('val_t_f1', t_f1, global_step=epoch)
        logger.add_scalar('val_t_acc', t_acc, global_step=epoch)
        logger.add_scalar('val_loss1', val_loss1, global_step=epoch)
        logger.add_scalar('val_f11', val_f11, global_step=epoch)
        logger.add_scalar('val_acc1', val_acc1, global_step=epoch)
        logger.add_scalar('val_t_f11', t_f11, global_step=epoch)
        logger.add_scalar('val_t_acc1', t_acc1, global_step=epoch)
...
def val(args):
    ...
    model.load_state_dict(torch.load("best_w.pth", map_location='cpu')['state_dict'])
    model = model.to(device)
    criterion = nn.CrossEntropyLoss()
    val_datasett = PTBXLTestClsdataset('ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/',
                                         True, choose=None, 
                                         train_lis=['work_dir/LAVQ_Editor/ecg',])
