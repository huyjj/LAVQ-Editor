...
def train_epoch(
    Editor, epoch, logger, criterion, train_dataloader, model_save_dir, show_interval=10
):
    Editor.gen.train()
    Editor.dis.train()
    ...
    for idx, batch in enumerate(train_dataloader):
        ...
        d_loss, d_ori_loss, d_cls_loss = Editor.optimize_discriminator(
            ab_ecg, ab_descrip, normal_ecg, norm_descrip, 10, 1
        )
        g_loss, g_f_loss, g_rec_loss, g_cls_loss, g_q_loss, indices = (
            Editor.optimize_generator(
                ab_ecg, ab_descrip, normal_ecg, norm_descrip, 10, 1
            )
        )

        logger.add_scalar(
            "D_loss", d_loss, global_step=idx + (epoch - 1) * len(train_dataloader)
        )
        logger.add_scalar(
            "D_cls_loss",
            d_cls_loss,
            global_step=idx + (epoch - 1) * len(train_dataloader),
        )
        logger.add_scalar(
            "D_ori_loss",
            d_ori_loss,
        ...
        logger.add_scalar(
            "G_loss", g_loss, global_step=idx + (epoch - 1) * len(train_dataloader)
        )
        logger.add_scalar(
            "G_gan_loss", g_loss, global_step=idx + (epoch - 1) * len(train_dataloader)
        )
        logger.add_scalar(
            "G_cls_loss",
            g_cls_loss,
        ...
        logger.add_scalar(
            "G_q_loss", g_q_loss, global_step=idx + (epoch - 1) * len(train_dataloader)
        )
        ...
        if idx != 0 and idx % show_interval == 0:
            ...
            print_log(
                "Iter %d,G_loss:%.3e, D_loss:%.3e" % (idx, g_loss, d_loss),
                model_save_dir,
            )
            print_log(
                f"G_q_loss:{g_q_loss} G_cls_loss:{g_cls_loss}, G_ori_loss:{g_f_loss}, G_rec_loss:{g_rec_loss}",
                model_save_dir,
            )
            print_log(
                f"D_cls_loss:{d_cls_loss}, D_ori_loss:{d_ori_loss}", model_save_dir
            )

    return


def val_epoch(Editor, epoch, val_dataloader, save_dir="./"):
    Editor.gen.eval()
    Editor.dis.eval()
    with torch.no_grad():
        for idx, batch in enumerate(val_dataloader):
            ...
            # forward
            disease, const_disease, q_loss_s, prefix_s, mask = Editor.disease_model(
                ab_ecg[:, :12], ab_ecg[:, 12:], ab_descrip
            )
            disease_r, const_r, q_loss_r, prefix_r, _ = Editor.disease_model(
                normal_ecg[:, :12], normal_ecg[:, 12:], norm_descrip
            )
        ...
        if epoch % 50 == 0 and epoch > 10:
            ...
            utils.save_ecg_image(
                normal_ecg[:5, :12], f"{save_dir}/epoch_ecg_{epoch}_input/"
            )
    return
    ...
    for ind, batch_norm in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(
                val_dataloader
            ):  # choose some item for generate ecg
                if num_per >= 50:
                    break
                ab_ecg = batch[0].to(device).float()
                ab_descrip = batch[1]
                ...
                disease_r, const_r, q_loss_r, prefix_r, _ = Editor.disease_model(
                    normal_ecg[:, :12], normal_ecg[:, 12:], normal_des
                )
...
def train(args):
    model_save_dir = os.path.join(
        args.work_dir, args.model_name, time.strftime("%Y%m%d%H%M")
    )
    ...
    model = VQSeparator(
        ...
        context_dim=1024,
        resolution=4096,
        language_model="model/Bio_ClinaBert",
    )
    if args.clip_dir:
        model.load_state_dict(
            torch.load(args.clip_dir, map_location="cpu")["state_dict"]
        )
    ...
    if args.resume:
        ...
        d_state = torch.load(
            f"{args.resume}/current_d.pth", map_location=torch.device("cpu")
        )
    ...
    train_dataset = ECGDataset(
        args.data_root + "reprepared/", True, classifier=True, choose_norm=True
    )
    ...
    print_log(
        f"train_datasize {len(train_dataset)} val_datasize {len(val_dataset)}",
        model_save_dir,
    )
    ...
    for epoch in range(start_epoch, args.max_epoch + 1):
        ...
        state, d_state, f_state = Editor.save_ckpt(epoch, stage)
        save_ckpt(state, d_state, f_state, True, model_save_dir)
        if epoch in args.stage_epoch:
            ...
            Editor.dis.load_state_dict(
                torch.load(best_w.replace(".pth", "_d.pth"))["state_dict"]
            )
            print_log(
                f"*" * 10 + "step into stage%02d lr %.3ef" % (stage, lr), model_save_dir
            )
...
def Gen_test(args):
    ...
    if args.clip_dir:
        ...
        print(f"Model Load from {args.clip_dir}....")
    classifier = EcgClassifer(
        classifer=args.classifer, num_classes=5, load_pretrain="best_w.pth"
    )
    ...
    # generator
    test_dataset = PTBXLTestClsdataset(
        args.data_root + "reprepared/",
        True,
        choose=[
            "NORM",
        ],
        train_lis=[
            "ptb-xl-a-large-publicly-available-electrocardiography-dataset-1.0.3/reprepared/Patient_Select_145_sclc_X_half1"
        ],
    )  # choose normal
    test_dataloader = DataLoader(
        test_dataset, batch_size=1, num_workers=args.num_workers, drop_last=True
    )
    print_log(
        f"train_datasize {len(train_dataset)} test_datasize {len(test_dataset)}",
        model_save_dir,
    )

    print(f"Start Generate......")
    val_loss, val_f1, val_acc = GenerateECG(
        Editor,
        classifier,
        logger,
        test_dataloader,
